...
/**
 * PermissionComponent::exportPdf() existed with no automated coverage at
 * all — not even the authorization gate — before this test. Mirrors the
 * Teacher/AffinityCatalog/TeacherAssignment export test pattern.
 */
class PermissionExportTest extends TestCase
{
    use RefreshDatabase;

    public function test_a_user_with_permission_can_download_the_pdf(): void
    {
        $this->actingAs($this->userWithPermissionExportPermission());
        ...
        Livewire::test(PermissionComponent::class)
            ->call('exportPdf')
            ->assertFileDownloaded('permisos.pdf', contentType: 'application/pdf');

        $this->assertStringContainsString(__('Permissions'), $fake->html);
    }

    public function test_a_user_without_permission_cannot_download_the_pdf(): void
    {
        // PermissionComponent::mount() itself gates on 'viewAny'
        // (permissions.view), so this user needs that much to reach the
        // page at all — proving exportPdf() has its own, narrower gate
        // (permissions.export_pdf), not just inheriting viewAny's.
        $this->actingAs($this->userWithPermissionViewOnlyPermission());
    ...
    public function test_the_real_pdf_pipeline_renders_a_valid_pdf(): void
    {
        ...
        $response = Livewire::test(PermissionComponent::class)->call('exportPdf');

        $response->assertFileDownloaded('permisos.pdf', contentType: 'application/pdf');
    ...
    private function userWithPermissionExportPermission(): User
    {
    ...
    private function userWithPermissionViewOnlyPermission(): User
    {
        $user = User::factory()->create();
