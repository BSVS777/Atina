...
return [
     ...
     *
     * Every PDF export in this app (Src\Shared\Export\Infrastructure\SpatiePdfExporter)
     * renders one static, server-side Blade table report with no
     * JavaScript/browser-only rendering requirement, so DOMPDF (pure PHP,
     * no Node/Chrome/Puppeteer) is the default. Switch a specific export
     * to a browser-backed driver only if it genuinely needs one, via
     * ->driver('browsershot') at that call site — not by changing this
     * default, which would put every export back behind Chrome.
     */
