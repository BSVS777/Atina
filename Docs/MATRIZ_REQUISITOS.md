...
|---|---|---|---|---|---|---|
| DO-01-F1 | Almacenar atestados académicos del docente (grado, institución, año, área de especialización) | Perfil del docente muestra lista completa de atestados | `AtestadoAcademico` (entity, `src/Docencia/Domain/Docente/`), `AtestadoRepository` (puerto), `RegistrarAtestadoAcademico`/`EditarAtestadoAcademico` (casos de uso), `EloquentAtestadoRepository` (adaptador), UI Livewire `pages::docencia.docente-perfil` | Unit: invariantes de `AtestadoAcademico`, `AnioObtencion` (RN-03), casos de uso con repos en memoria. Feature: alta/edición vía Livewire | `tests/Unit/Docencia/*`, `tests/Feature/Docencia/DocentePerfilLivewireTest.php` | Implementado |
| DO-01-F2 | Restringir modificación de atestados a rol Administrador o Coordinadora de Docencia | Un rol distinto intentando modificar es rechazado | `PoliticaAutorizacionAtestado` (dominio, permiso `atestados.gestionar`), `App\Models\Role`/`Permission`, `User::permisos()`, Gate `gestionar-atestados` (adaptador delgado en `AppServiceProvider`) | Unit: política rechaza actor sin el permiso. Feature: acción Livewire sin permiso → 403 (`assertForbidden`) | `tests/Unit/Docencia/PoliticaAutorizacionAtestadoTest.php`, `tests/Feature/Docencia/AtestadoAuthorizationTest.php` | Implementado |
| DO-01-F3 | Registrar auditoría de toda modificación (usuario, fecha, campo, valor anterior, valor nuevo) | Cada modificación queda en log de auditoría con esos 5 campos | `AuditLogEntry` (entity, `src/Docencia/Domain/Auditoria/`), `AuditLogRepository` (puerto), `EloquentAuditLogRepository` (adaptador → tabla `auditorias`) | Unit: casos de uso emiten entrada de auditoría correcta (creación completa, edición solo con campos cambiados). Feature: fila persistida en BD tras alta/edición vía Livewire | `tests/Unit/Docencia/RegistrarAtestadoAcademicoTest.php`, `EditarAtestadoAcademicoTest.php`, `tests/Feature/Docencia/AtestadoAuditoriaTest.php` | Implementado |
...
| RN-02 | Área de especialización es obligatoria y debe ser comparable con el vocabulario del catálogo (DO-02) | VO `Especialidad` (`src/Docencia/Domain/Docente/Especialidad.php`, id + nombre de la tabla `especialidades`) — **decisión de diseño compartida con DO-02**, ver §6. Renombrada de "AreaEspecializacion" a "Especialidad" al implementar, para coincidir con el nombre real de la tabla/columna del schema compartido |
| RN-03 | Año de obtención no puede ser futuro ni implausible | VO `AnioObtencion` (validación de rango 1950–año actual) |
| RN-04 | Solo Administrador o Coordinadora de Docencia modifican atestados | `PoliticaAutorizacionAtestado::puedeGestionar()` (dominio, PHP puro) — expresada como el permiso `atestados.gestionar` del RBAC compartido, no como nombres de rol hardcodeados. El Gate `gestionar-atestados` (`AppServiceProvider`) es un adaptador delgado que solo traduce `User::permisos()` a la llamada de dominio |
| RN-05 | Toda modificación (alta o edición) genera auditoría; los intentos rechazados por rol **no** generan auditoría según el enunciado (solo se audita la modificación efectiva) | `RegistrarAtestadoAcademico`/`EditarAtestadoAcademico` (Application) — la edición además solo audita los campos que realmente cambiaron, no el registro completo |
